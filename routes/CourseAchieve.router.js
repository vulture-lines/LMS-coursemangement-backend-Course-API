...
const User = require("../models/User.model");
const authenticate = require("../middleware/auth");
const generateCertificateId = require("../middleware/generateCertificateId");
...
// Get all achievements (mentor only)
router.get("/", authenticate, requireMentor, async (req, res) => {
  try {
    const data = await CourseAchievement.find().sort({ createdAt: -1 });
    res.json(data);
...
router.post("/", authenticate, requireMentor, async (req, res) => {
  try {
    const { userId, courseId, mentorName, authorityName, qrCodeUrl } = req.body;
    ...
    const certificateId = await generateCertificateId(completedAt);
...
// Update an existing course achievement
router.put("/:id", authenticate, requireMentor, async (req, res) => {
  try {
    const { userId, courseId, mentorName, authorityName, qrCodeUrl } = req.body;
